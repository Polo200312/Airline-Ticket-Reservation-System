...
export async function getTermCourseList(
  numName: string | null
): Promise<CourseItem[]> {
  const res = await generalRequest("/api/course/getTermCourseList", {
    numName: numName,
  });
  return res.data as CourseItem[];
}
export async function getPersonalCourseList(
  ...
  const res = await generalRequest("/api/personalCourse/getPersonalCourseList", {
    numName: numName,
  });
  return res.data as CourseItem[];
}
export async function getPersonalCourseTable(
  numName: string | null,
): Promise<CourseTable[]> {
  const res = await generalRequest("/api/personalCourse/getPersonalCourseTable", {
    numName: numName,
  });
  return res.data as CourseTable[];
}
//删除课程后台数据请求方法
...
export async function personalCourseDelete(courseId: number): Promise<DataResponse> {
  const res = await generalRequest("/api/personalCourse/personalCourseDelete", {
    courseId: courseId,
  });
  return res as DataResponse;
}
...
export async function personalCourseSave(data: CourseItem): Promise<DataResponse> {
  const res = await generalRequest("/api/personalCourse/personalCourseSave", data);
  return res as DataResponse;
}
//获取学生选择项列表后台数据请求方法
...
export async function getTeacherCourseItemOptionList(): Promise<OptionItem[]> {
  const res = await generalRequest("/api/score/getTeacherCourseItemOptionList", null);
  return res.itemList as OptionItem[];
}
//获取成绩列表后台数据请求方法
...
export async function getTeacherScoreList(
  ...
  const res = await generalRequest("/api/score/getTeacherScoreList", {
    studentId: studentId,
    courseId: courseId,
  });
  return res.data as [];
