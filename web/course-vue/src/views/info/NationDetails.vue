    ...
    <el-card class="box-card">
      <div slot="header" class="header-section clearfix">
        <span class="header-title"
          ><el-icon><View /></el-icon>{{ info.nationName }} - 详情</span
        >
        <div class="button-group">
          <el-button
            ...
            @click="editNation"
          >
            <el-icon><edit-pen /></el-icon>编辑国家信息
          </el-button>
...
export default defineComponent({
  ...
  methods: {
    async query() {
      const nationIdStr = this.$route.query.nationId as string;
      this.nationId = Number(nationIdStr);
      let res = await getNationInfo(this.nationId);
      this.info = res.data.info;
      console.log(this.info);
    },
    editNation() {
      this.$router.push({ path: '/EditNation', query: { nationId: this.info.nationId, nationName: this.info.nationName } });
    },
    back(){
      this.$router.back();
    },
