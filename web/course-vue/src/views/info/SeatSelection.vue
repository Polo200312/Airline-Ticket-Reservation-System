...
methods: {
  ...
  initSeatMap() {
    ...
    for (let row = 1; row <= rows; row++) {
      ...
      for (let col = 0; col < 6; col++) {
        ...
        // 查找对应座位信息
        const seatInfo = this.seatList.find(seat => seat.seatRowNumber === rowLabel && seat.seatColumnNumber === columnLabel && seat.isBooked === true);
  ...
  selectSeat(row: string, column: string) {
    ...
    async reBook() {
      console.log('开始改签！', this.selectedSeatRow, '排', this.selectedSeatColumn, '座');
      if (this.selectedSeatRow !== null && this.selectedSeatColumn !== null) {
        const selectedSeat = this.seatList.find(seat => seat.seatRowNumber === this.selectedSeatRow && seat.seatColumnNumber === this.selectedSeatColumn);

        if (selectedSeat) {
          this.seatId = selectedSeat.seatId;
          console.log(this.seatId + "-" + this.dealId + "-" + this.planeId);
          // 将数据发送到后端
          let res = await reBook({
            seatId: this.seatId,
