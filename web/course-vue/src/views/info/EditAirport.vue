  ...
  <div class="edit-client">
    <el-card class="box-card">
      <div slot="header" class="clearfix">
        <span
          ><el-icon><Checked /></el-icon>编辑机场信息</span
        >
      </div>
      <el-form ref="flightForm" label-width="120px" class="new-client-form">
        ...
        <el-form-item label="所属城市">
          <el-input v-model="cityName" placeholder="请输入时区"></el-input>
        </el-form-item>
        ...
        <el-form-item label="经度">
          <el-input v-model="airportY" placeholder="请输入城市名称"></el-input>
        </el-form-item>
...
export default defineComponent({
  ...
  methods: {
    async query() {
      ...
      this.name = this.$route.query.airportName as string;
      let res = await getAirportInfo(this.airportId);
      this.info = res.data.info;
      this.name = this.info.airportName;
      this.airportX = this.info.airportX;
      this.airportY = this.info.airportY;
      this.airportStatus = this.info.status;
      this.airportIntroduction = this.info.airportIntroduction;
      this.cityName = this.info.cityName;
    },
    async submitForm() {
      // 提交表单的逻辑，这里可以发送请求将数据提交到后端
      if (this.name == "" || this.name == undefined) {
        message(this, "机场名称为空,请填写！");
        return;
      }
      ...
      let res= await changeAirportSave({
